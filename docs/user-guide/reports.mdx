...
These reports can be categorized based on different parameters in various sections such as: **Favorites, My Folders, Hub Folders**, and **BI Platform**.
...
* Large tiles display the report's thumbnail along with other details.
...
* Small tiles display all the report's details except for the thumbnail.
...
Following are the report's fields displayed:
...
You can only view the report's details ( <img width="25" src={useBaseUrl('/img/details-icon.png')} /> ) along with the custom and user-defined attributes on the report.
...
To refresh the report, open the report and select **Reload report** ( <img width="25" src={useBaseUrl('/img/reload.png')} /> ).

### 7. Add to My Folders

You can add any accessible report(s) to the folders created in the 'My Folders' section in two ways:

1. Search and open any report, then select **Add to My Folders** ( <img width="25" src={useBaseUrl('/img/add-to.png')} /> ) icon.
   * In the **Move Item(s)** pop-up window, select the directories listed and then select **Move Here**.
1. Select a report(s)/folder(s) from any section **Dashboard/My Folders/Hub Folders/BI Platform/History**, and then select **Add to My Folders**.
   * Next, select any of the 'My Folders' directory and then select **Add Here**.
