...
Click on the report and the report either open within BI Hub or in a separate tab in the browser ( if you have toggled the <Link to={useBaseUrl('docs/user-guide/settings')}>Open report in new tab</Link> option ON ).

#### Multi-tab view

You can open multiple reports at a time and switch between the tabs within the BI Hub application.
