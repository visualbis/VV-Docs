...
1. Click the **Start** button and select **All Programs > Accessories > Command Prompt**.
Right-click Command Prompt and select **Run as administrator**.
1. To enable the trusted host, run the following commands in the command prompt. 

```bash title="<Tableau Server Installation Location>\Tableau\Tableau Server\<version>\bin"
 $ tabadmin stop
