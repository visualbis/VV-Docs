...
> - If the user names are different between BI platforms, to import users from the other BI platform(s), Click Save and proceed to Section <Link to={useBaseUrl('docs/admin-guide/user-manager/user-management#add-users-from-other-bi-platforms')}>Add Users from other BI Platforms (users having different user names across BI Platforms)</Link>.
...
A list of Reports are displayed as:
...
### Encrypted Password non-updatable

The password field is now encrypted for the agents in the **Edit Instance** screen. Admin cannot update the password here.

  <div class="center">
    <Zoom>
      <img alt="Password update not allowed" src={useBaseUrl('doc-images/admin-guide/agent/password-update-fail.png')}/>
    </Zoom>
    <p>Password update not allowed</p>
  </div>

> Only a Server Admin can reset an agent's password property file located in the `<agent installation directory>`.
