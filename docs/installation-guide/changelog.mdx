...
#### New
* Set the native user <Link to={useBaseUrl('docs/admin-guide/settings/general#security')}>Password Strength Validator</Link>.
* User can now prefer to <Link to={useBaseUrl('docs/user-guide/settings#preferences')}>choose the Landing Page</Link>.

#### Enhancements
* Enabled <Link to={useBaseUrl('docs/admin-guide/agent-manager/agent/agent-management#sync-failure-mail-notification')}>email notifications for failed Sync jobs(Reports/Users)</Link>.
* Agent's edit Instance configuration screen now encrypts the password inside the agent and restricts updating the password. [Refer the section <Link to={useBaseUrl('docs/admin-guide/agent-manager/agent/agent-management#encrypted-password-non-updatable')}>Encrypted Password non-updatable</Link>].
...
* Agent Sync Users flow now includes Windows AD Authentication
